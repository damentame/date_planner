...
// Import utils
import { syncAllTasks } from './utils/taskSync';
...
function App() {
  ...
  useEffect(() => {
    const loadTasks = async () => {
      try {
        setIsLoading(true);
        const fetchedTasks = await syncAllTasks();
        setTasks(fetchedTasks);
        setIsLoading(false);
      } catch (err) {
        setError('Failed to load tasks. Please check your connection to the Notion API.');
        setIsLoading(false);
